import {Render360Runtime,fmtHex} from './runtime/render360-runtime.js?v=43';
...
function applyAppSettings(){
  ...
  const themeColor=theme==='light'?'#f2f2f7':'#000000';document.querySelector('meta[name="theme-color"]')?.setAttribute('content',themeColor);
  document.documentElement.style.setProperty('--controller-alpha',String(Number(appSettings.controllerOpacity||.14)));
  document.documentElement.style.setProperty('--controller-scale',String(Number(appSettings.controllerScale||1)));
...
function startRelink(){if(!currentGame)return;relinkTarget=currentGame;const input=$('relinkInput');input.value='';input.click();}
async function handleRelink(file){
  ...
  try{
    ...
    runtime.bindSource(relinkTarget.id,source);currentGame=relinkTarget;relinkTarget=null;await renderLibrary();await renderDetail();await playCurrent();
...
function leaveGame(){stopTitle();$('controllerLayer').classList.remove('paused');closeSheets();setState('GAME_DETAILS');renderDetail();}
...
async function clearStoredGames(){await clearGamesDirectory();for(const game of games){if(String(game.opfsPath||'').startsWith('Render360/Games/')){game.opfsPath=null;game.persistentSource=false;game.needsRelink=true;runtime.unbindSource(game.id);await putGame(game);}}await refreshLibrary();await updateStorageUi();}
...
$('settingsButton').addEventListener('click',()=>{renderAppSettings();setState('APP_SETTINGS');});$('appSettingsBack').addEventListener('click',()=>setState('LIBRARY'));$('detailBack').addEventListener('click',()=>{setState('LIBRARY');renderLibrary();});
$('playGameButton').addEventListener('click',playCurrent);$('gameSettingsButton').addEventListener('click',()=>{renderGameSettings();setState('GAME_SETTINGS');});$('gameSettingsBack').addEventListener('click',()=>setState('GAME_DETAILS'));$('saveGameSettings').addEventListener('click',saveGameSettings);$('resetGameSettings').addEventListener('click',()=>{resetTitleProfile(currentGame);renderGameSettings();});
$('chooseCoverButton').addEventListener('click',()=>{$('coverInput').value='';$('coverInput').click();});$('coverInput').addEventListener('change',e=>chooseCover(e.target.files?.[0]));$('relinkGameButton').addEventListener('click',startRelink);$('relinkInput').addEventListener('change',e=>handleRelink(e.target.files?.[0]));$('deleteGameButton').addEventListener('click',()=>showAlert('Remove Game?',`Remove ${currentGame?.name||'this game'} from Render360? Stored browser copies and cached cover art for this entry will also be removed.`,[{label:'Cancel'},{label:'Remove',action:removeCurrentGame}]));
$('diagnosticsButton').addEventListener('click',showDiagnostics);$('appDiagnosticsButton').addEventListener('click',showDiagnostics);$('pauseButton').addEventListener('click',pauseGame);$('resumeButton').addEventListener('click',resumeGame);$('leaveGameButton').addEventListener('click',leaveGame);$('runtimeBack').addEventListener('click',pauseGame);
$('createGamesFolder').addEventListener('click',createStorageFolder);$('requestPersistentStorage').addEventListener('click',askPersistentStorage);$('clearGameStorage').addEventListener('click',()=>showAlert('Clear Game Copies?',`Delete files stored inside Render360/Games? Library entries stay, but affected games will need to be selected again.`,[{label:'Cancel'},{label:'Clear',action:clearStoredGames}]));$('resetAppSettings').addEventListener('click',()=>showAlert('Reset Settings?',`Restore Render360 global settings to their defaults?`,[{label:'Cancel'},{label:'Reset',action:()=>{appSettings=resetAppSettings();applyAppSettings();renderAppSettings();}}]));
$('scrim').addEventListener('click',()=>{if(appState==='PAUSED')return;closeSheets();});document.querySelectorAll('[data-close-sheet]').forEach(b=>b.addEventListener('click',closeSheets));bindAppSettingsEvents();wireDigitalControls();wireStick($('leftStick'),$('leftStickKnob'));
window.addEventListener('keydown',e=>{const map={Enter:'START',Escape:'BACK',q:'LB',e:'RB',z:'LT',c:'RT',x:'X',y:'Y',a:'A',b:'B'};const key=map[e.key]||map[e.key?.toLowerCase?.()];if(key)runtime.setKey(key,true);});window.addEventListener('keyup',e=>{const map={Enter:'START',Escape:'BACK',q:'LB',e:'RB',z:'LT',c:'RT',x:'X',y:'Y',a:'A',b:'B'};const key=map[e.key]||map[e.key?.toLowerCase?.()];if(key)runtime.setKey(key,false);});
...
async function boot(){
  ...
  try{
    await runtime.init();await restorePersistentSources();const c=runtime.contract();$('runtimeSyncStatus').classList.add('ready');setText('runtimeSyncText',`Runtime synced · Core V${c.loadedCoreBuild} · ABI ${fmtHex(c.loadedAbi)}`);setText('aboutCore',`V${c.loadedCoreBuild} · minimum V${c.minCoreBuild}`);setText('aboutAbi',fmtHex(c.loadedAbi));$('importButton').disabled=false;$('emptyImportButton').disabled=false;log('ok',`Render360 41 ready · Core V${c.loadedCoreBuild} · ABI ${fmtHex(c.loadedAbi)} · ISO/XEX/STFS launch adapters active`);
