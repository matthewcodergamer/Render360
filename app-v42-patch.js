...
function coverSurface(url,label='Xbox 360 game cover'){const surface=document.createElement('span');surface.className='cover-art-surface';surface.setAttribute('role','img');surface.setAttribute('aria-label',label);surface.setAttribute('draggable','false');surface.style.backgroundImage=`url(${JSON.stringify(String(url)).slice(1,-1)})`;return surface;}
function convertNativeCoverImages(root=document){root.querySelectorAll?.('.cover-shell img,.detail-cover img').forEach(img=>{const src=img.currentSrc||img.src;if(!src){img.remove();return;}img.replaceWith(coverSurface(src,img.alt||'Xbox 360 game cover'));});}
function setCoverSurface(container,url,label){if(!container||!url)return;container.querySelectorAll('img,.cover-art-surface').forEach(node=>node.remove());container.querySelector('.cover-placeholder')?.remove();container.prepend(coverSurface(url,label));}
function applyCachedCover(tile){const url=coverUrls.get(tile.dataset.gameId);if(!url)return;const shell=tile.querySelector('.cover-shell');if(!shell)return;const title=tile.querySelector('.game-tile-title')?.textContent||'Xbox 360 game';setCoverSurface(shell,url,`${title} cover`);}
function decorateTile(tile){if(!(tile instanceof HTMLElement))return;const shell=tile.querySelector('.cover-shell');if(!shell)return;convertNativeCoverImages(shell);if(!shell.querySelector('.tile-play-badge'))shell.appendChild(playBadge());if(!tile.querySelector('.game-tile-hint')){const hint=document.createElement('span');hint.className='game-tile-hint';hint.textContent='Tap to play · hold for details';tile.appendChild(hint);}const title=tile.querySelector('.game-tile-title')?.textContent||'Game';tile.setAttribute('aria-label',`${title}. Tap to play. Press and hold for game details.`);tile.setAttribute('draggable','false');applyCachedCover(tile);}
...
function bindLibraryLaunchGestures(){
  const grid=$('gameGrid');if(!grid)return;const ownedTile=event=>event.target.closest?.('.game-tile')||null,suppressNative=event=>{if(ownedTile(event))event.preventDefault();};
  grid.addEventListener('dragstart',suppressNative,true);grid.addEventListener('selectstart',suppressNative,true);grid.addEventListener('webkitmouseforcewillbegin',suppressNative,true);
  grid.addEventListener('pointerdown',event=>{const tile=ownedTile(event);if(!tile)return;if(event.pointerType==='mouse'&&event.button!==0)return;const state={timer:null,held:false,startX:event.clientX,startY:event.clientY,pointerId:event.pointerId,blockTrustedUntil:0,allowSyntheticDetails:false};state.timer=setTimeout(()=>{state.timer=null;state.held=true;tile.classList.add('v42-holding');navigator.vibrate?.(10);try{tile.setPointerCapture?.(event.pointerId);}catch{}},500);holdState.set(tile,state);},true);
  grid.addEventListener('pointermove',event=>{const tile=ownedTile(event);if(!tile)return;const state=holdState.get(tile);if(!state||state.pointerId!==event.pointerId||state.held)return;if(Math.hypot(event.clientX-state.startX,event.clientY-state.startY)>12)clearHoldTimer(state);},true);
  const endPointer=event=>{const tile=ownedTile(event);if(!tile)return;const state=holdState.get(tile);if(!state||state.pointerId!==event.pointerId)return;clearHoldTimer(state);if(state.held){event.preventDefault();event.stopPropagation();state.blockTrustedUntil=Date.now()+900;state.allowSyntheticDetails=true;try{tile.releasePointerCapture?.(event.pointerId);}catch{}queueMicrotask(()=>tile.click());setTimeout(()=>tile.classList.remove('v42-holding'),160);}};
  grid.addEventListener('pointerup',endPointer,true);grid.addEventListener('pointercancel',event=>{const tile=ownedTile(event),state=tile&&holdState.get(tile);clearHoldTimer(state);tile?.classList.remove('v42-holding');},true);
  grid.addEventListener('click',event=>{const tile=ownedTile(event);if(!tile)return;const state=holdState.get(tile);if(state?.allowSyntheticDetails&&!event.isTrusted){state.allowSyntheticDetails=false;return;}if(event.isTrusted&&state?.blockTrustedUntil>Date.now()){event.preventDefault();event.stopImmediatePropagation();return;}if(!event.isTrusted)return;scheduleAutoPlay(tile);},true);
  grid.addEventListener('contextmenu',event=>{const tile=ownedTile(event);if(!tile)return;event.preventDefault();event.stopImmediatePropagation();const state=holdState.get(tile)||{};clearHoldTimer(state);state.allowSyntheticDetails=true;state.blockTrustedUntil=Date.now()+900;holdState.set(tile,state);queueMicrotask(()=>tile.click());},true);
}
function bindGlobalCoverProtection(){const app=$('app');if(!app)return;const block=event=>{const target=event.target.closest?.('.game-tile,.cover-shell,.detail-cover,.cover-art-surface');if(!target)return;if(event.type==='contextmenu'&&target.closest?.('.game-tile'))return;event.preventDefault();};app.addEventListener('dragstart',block,true);app.addEventListener('selectstart',block,true);app.addEventListener('contextmenu',event=>{if(event.target.closest?.('.detail-cover,.cover-art-surface')&&!event.target.closest?.('.game-tile'))event.preventDefault();},true);}

function closePatchAlert(){$('iosAlert')?.classList.add('hidden');$('scrim')?.classList.add('hidden');}
function patchAlert(title,message,actions=[{label:'OK'}]){const alert=$('iosAlert'),wrap=$('alertActions');if(!alert||!wrap)return;if($('alertTitle'))$('alertTitle').textContent=title;if($('alertMessage'))$('alertMessage').textContent=message;wrap.innerHTML='';wrap.style.gridTemplateColumns=`repeat(${Math.min(2,actions.length)},1fr)`;for(const action of actions){const button=document.createElement('button');button.textContent=action.label;button.addEventListener('click',async()=>{if(button.disabled)return;button.disabled=true;if(!action.keepOpen)closePatchAlert();try{await action.action?.();}catch(error){console.error('[Render360 V44]',error);setTimeout(()=>patchAlert('Action Failed',error?.message||String(error),[{label:'OK'}]),0);}},{once:true});wrap.appendChild(button);}$('scrim')?.classList.remove('hidden');alert.classList.remove('hidden');}
