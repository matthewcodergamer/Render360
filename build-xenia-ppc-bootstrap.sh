...
LLVM_INCLUDE="$(llvm-config --includedir 2>/dev/null || true)"; if [ -n "$LLVM_INCLUDE" ] && [ -d "$LLVM_INCLUDE" ]; then COMMON+=("-I$LLVM_INCLUDE"); echo "LLVM headers: $LLVM_INCLUDE"; fi

SOURCES=(
  "third_party/fmt/src/format.cc" "src/xenia/base/arena.cc" "src/xenia/base/cvar.cc" "src/xenia/base/utf8.cc"
  "src/xenia/base/filesystem_posix.cc" "src/xenia/base/memory_posix.cc" "src/xenia/base/mapped_memory_posix.cc" "src/xenia/base/mutex.cc" "src/xenia/base/string.cc" "src/xenia/base/string_buffer.cc"
...
classify_failure(){ local log="$1"; if grep -Eqi 'Instruction pointer not specified|target CPU architecture|x64_backend|x64|amd64|avx|sse|m128|m256|xbyak|executable.*memory|code.?cache' "$log"; then echo HOST_ARCH_DEPENDENCY; elif grep -Eqi 'static assertion.*64b padded|sizeof\(PPCContext\)' "$log"; then echo PPC_CONTEXT_ABI_DEPENDENCY; elif grep -Eqi 'CreateFileMapping|MapView|file.?mapping|4gb|address space|windows\.h|win32|VirtualAlloc|sys/mman|mmap|shm_open|ftruncate64|mprotect|munmap' "$log"; then echo HOST_MEMORY_MAPPING_DEPENDENCY; elif grep -Eqi 'static assertion|error:' "$log"; then echo CXX_OR_PORTABILITY_DEPENDENCY; else echo CXX_OR_PORTABILITY_DEPENDENCY; fi; }
passed=0; failed=0; : > "$OUT/report.tsv"; printf 'source\tresult\tclassification\n' >> "$OUT/report.tsv"
compile_one(){ local label="$1" src="$2" obj="$OUT/$(echo "$label"|tr '/' '_').o" log="$obj.log"; printf '[WASM32] %-64s ' "$label"; if "$CXX" "${COMMON[@]}" -c "$src" -o "$obj" >"$log" 2>&1; then echo PASS; printf '%s\tPASS\tPORTABLE\n' "$label" >> "$OUT/report.tsv"; passed=$((passed+1)); else local category="$(classify_failure "$log")"; echo "BLOCKED ($category)"; printf '%s\tBLOCKED\t%s\n' "$label" "$category" >> "$OUT/report.tsv"; failed=$((failed+1)); fi; }
for rel in "${SOURCES[@]}"; do case "$rel" in "src/xenia/base/arena.cc") compile_one "$rel" "$OVERLAY/xenia/base/arena.cc";; "src/xenia/base/cvar.cc") compile_one "$rel" "$OVERLAY/xenia/base/cvar.cc";; "src/xenia/base/utf8.cc") compile_one "$rel" "$OVERLAY/xenia/base/utf8.cc";; "src/xenia/memory.cc") compile_one "$rel" "$OVERLAY/xenia/memory.cc";; "src/xenia/cpu/mmio_handler.cc") compile_one "$rel" "$OVERLAY/xenia/cpu/mmio_handler.cc";; "src/xenia/cpu/processor.cc") compile_one "$rel" "$OVERLAY/xenia/cpu/processor.cc";; *) compile_one "$rel" "$XENIA/$rel";; esac; done
...
echo; echo "Xenia PPC/HIR wasm32 compile matrix: $passed passed, $failed blocked"; echo "Report: $OUT/report.tsv"; if [ "$failed" -ne 0 ]; then exit 1; fi; if [ "$passed" -eq 0 ]; then exit 1; fi
