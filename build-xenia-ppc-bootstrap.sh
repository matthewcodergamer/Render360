...
classify_failure(){local log="$1";if grep -Eqi 'Instruction pointer not specified|target CPU architecture|x64_backend|x64|amd64|avx|sse|m128|m256|xbyak|executable.*memory|code.?cache' "$log";then echo HOST_ARCH_DEPENDENCY;elif grep -Eqi 'static assertion.*64b padded|sizeof\(PPCContext\)' "$log";then echo PPC_CONTEXT_ABI_DEPENDENCY;elif grep -Eqi 'CreateFileMapping|MapView|file.?mapping|4gb|address space|windows\.h|win32|VirtualAlloc|sys/mman|mmap|shm_open|ftruncate64|mprotect|munmap' "$log";then echo HOST_MEMORY_MAPPING_DEPENDENCY;else echo CXX_OR_PORTABILITY_DEPENDENCY;fi;}
passed=0;failed=0;:>"$OUT/report.tsv";printf 'source\tresult\tclassification\n'>>"$OUT/report.tsv"
compile_one(){local label="$1";local src="$2";local obj="$OUT/$(echo "$label"|tr '/' '_').o";local log="$obj.log";printf '[WASM32] %-64s ' "$label";if "$CXX" "${COMMON[@]}" -c "$src" -o "$obj">"$log" 2>&1;then echo PASS;printf '%s\tPASS\tPORTABLE\n' "$label">>"$OUT/report.tsv";passed=$((passed+1));else local category;category="$(classify_failure "$log")";echo "BLOCKED ($category)";printf '%s\tBLOCKED\t%s\n' "$label" "$category">>"$OUT/report.tsv";failed=$((failed+1));fi;}
...
compile_one "render360/wasm_backend_fpu_probe.cpp" "$ROOT/src/xenia_web_bootstrap/wasm_backend_fpu_probe.cpp"
