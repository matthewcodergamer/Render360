...
classify_failure() {
  ...
  else echo CXX_OR_PORTABILITY_DEPENDENCY; fi
}
...
compile_c(){ local label="$1" src="$2" obj="$OUT/$(echo "$label"|tr '/' '_').o" log="$obj.log"; printf '[WASM32] %-64s ' "$label"; if "$CC" "${COMMON_C[@]}" -c "$src" -o "$obj" >"$log" 2>&1; then echo PASS; printf '%s\tPASS\tPORTABLE\n' "$label" >> "$OUT/report.tsv"; passed=$((passed+1)); else local category; category="$(classify_failure "$log")"; echo "BLOCKED ($category)"; printf '%s\tBLOCKED\t%s\n' "$label" "$category" >> "$OUT/report.tsv"; failed=$((failed+1)); fi; }
