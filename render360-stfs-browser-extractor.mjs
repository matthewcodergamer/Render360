// Render360 V43 STFS extraction compatibility path.
//
...
function layout(stfs){
  ...
  return {
    flags,
    blocksPerHashTable,
    step0:BLOCKS_L0+blocksPerHashTable,
    step1:BLOCKS_L1+((BLOCKS_L0+1)*blocksPerHashTable),
    dataBase:roundBlock(Number(stfs?.headerSize||0)),
    totalBlocks:Number(stfs?.totalBlockCount||0)>>>0,
  };
}
...
function blockOffset(blockIndex,l){
  ...
  for(let i=0;i<3;i++){
    block+=floorDiv(index+base,base)*l.blocksPerHashTable;
    if(index<base)break;
    base*=BLOCKS_L0;
  }
...
function hashBlockNumber(blockIndex,level,l){
  ...
  if(level===0){
    if(index<BLOCKS_L0)return 0;
    let block=floorDiv(index,BLOCKS_L0)*l.step0;
    block+=(floorDiv(index,BLOCKS_L1)+1)*l.blocksPerHashTable;
    return index<BLOCKS_L1?block:block+l.blocksPerHashTable;
  }
  if(level===1){
    if(index<BLOCKS_L1)return l.step0;
    return floorDiv(index,BLOCKS_L1)*l.step1+l.blocksPerHashTable;
  }
  return l.step1;
}

function hashOffset(blockIndex,level,l){
  return assertSafe(l.dataBase+hashBlockNumber(blockIndex,level,l)*BLOCK_SIZE,'STFS hash offset');
}
...
async function nextBlock(file,current,l,counter,maxRequests){
  ...
  if(l.totalBlocks>BLOCKS_L1){
    const table=await readExact(file,hashOffset(current,2,l)+secondary,BLOCK_SIZE,counter,maxRequests);
    const record=floorDiv(current,BLOCKS_L1)%BLOCKS_L0;
    const off=record*HASH_ENTRY_SIZE+HASH_INFO_OFFSET;
    if(off+4>table.length)throw new Error('STFS L2 hash record is out of bounds');
    secondary=(be32(table,off)&HASH_ACTIVE_INDEX_BIT)?BLOCK_SIZE:0;
  }
  if(l.totalBlocks>BLOCKS_L0){
    const table=await readExact(file,hashOffset(current,1,l)+secondary,BLOCK_SIZE,counter,maxRequests);
    const record=floorDiv(current,BLOCKS_L0)%BLOCKS_L0;
    const off=record*HASH_ENTRY_SIZE+HASH_INFO_OFFSET;
    if(off+4>table.length)throw new Error('STFS L1 hash record is out of bounds');
    secondary=(be32(table,off)&HASH_ACTIVE_INDEX_BIT)?BLOCK_SIZE:0;
  }
  const table=await readExact(file,hashOffset(current,0,l)+secondary,BLOCK_SIZE,counter,maxRequests);
  const record=current%BLOCKS_L0;
  const off=record*HASH_ENTRY_SIZE+HASH_INFO_OFFSET;
  if(off+4>table.length)throw new Error('STFS L0 hash record is out of bounds');
  return be32(table,off)&0xFFFFFF;
}

function snapshot({entry,current,bytesDone,blocksDone,status=1}){
  return {
    status,
    statusName:status===2?'Complete':'Working',
    entryIndex:entry.index>>>0,
    currentBlock:current>>>0,
    logicalOffset:bytesDone>>>0,
    bytesTotal:entry.length>>>0,
...
export async function extractStfsEntryBrowser(file,{entry,stfs,captureLimit=32*1024*1024,maxRequests=65536,onProgress=null}={}){
  ...
  const total=Number(entry.length||0)>>>0;
  const expected=Math.ceil(total/BLOCK_SIZE);
  const l=layout(stfs);
  ...
  if(total===0){
    ...
    return {...snapshot({entry,current:entry.startBlock||0,bytesDone:0,blocksDone:0,status:2}),complete:true,requestCount:0,totalBytesRead:0,captured,fullyCaptured:true,fallback:'browser-stfs-v32-semantics'};
  }
  let current=Number(entry.startBlock)>>>0;
  if(l.totalBlocks&&current>=l.totalBlocks)throw new Error('STFS file starts outside the package block range');
  const visited=new Set([current]);
  const counter={count:0,bytes:0};
  const captureBytes=Math.min(total,Math.max(0,Number(captureLimit)||0));
  const captured=new Uint8Array(captureBytes);
  let bytesDone=0,blocksDone=0;
  onProgress?.(snapshot({entry,current,bytesDone,blocksDone}));

  while(bytesDone<total){
    const size=Math.min(BLOCK_SIZE,total-bytesDone);
    const data=await readExact(file,blockOffset(current,l),size,counter,maxRequests);
    if(bytesDone<captured.length)captured.set(data.subarray(0,Math.min(data.length,captured.length-bytesDone)),bytesDone);
    bytesDone+=size;blocksDone++;
    onProgress?.(snapshot({entry,current,bytesDone,blocksDone}));
    if(bytesDone>=total)break;
    let next;
    if(entry.contiguous)next=(current+1)>>>0;
    else next=await nextBlock(file,current,l,counter,maxRequests);
  ...
  const done=snapshot({entry,current,bytesDone,blocksDone,status:2});
  onProgress?.(done);
...
export function browserStfsExtractorContract(){return {version:43,nativePreferred:true,blockSize:BLOCK_SIZE,hashLevels:3,failClosed:true};}
