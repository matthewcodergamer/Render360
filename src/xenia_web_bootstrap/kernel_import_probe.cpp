...
bool ResolveKernelImportThunk(uint32_t thunk_address) {
  for (const auto& entry : g_entries) {
    ...
    if (!entry.implemented) {
      g_last_status = 2;  // exact import reached but not implemented.
      return false;
    }
    if (!SetHIRCorrectnessActiveGPR(3, entry.r3_result)) {
      g_last_status = 3;  // not in active PPC execution.
      return false;
    }
    g_last_status = 1;
    return true;
...
uint32_t r360_kernel_import_register(uint32_t thunk_address,
                                    ...
                                    uint32_t r3_result) {
  return render360::xenia_web::RegisterKernelImportThunk(
             thunk_address, module_id, ordinal, implemented != 0, r3_result)
             ? 1u
             : 0u;
}
