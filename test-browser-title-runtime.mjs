...
const contract=browserTitleRuntimeContract();assert.equal(contract.bootstrapUrl,'./xenia_ppc_bootstrap.stable.wasm');assert.equal(contract.bootstrapMetadataUrl,'./xenia_ppc_bootstrap.stable.meta.json');assert.equal(contract.singletonBootstrap,true);assert.match(contract.bootstrapIntegrity,/SHA-256/);assert.equal(contract.wholeIsoCopy,false);assert.match(contract.input,/File\/Blob/);assert.equal(contract.growablePreparedPeStaging,true);assert.equal(contract.maxPreparedPeStagingBytes,256*1024*1024);
