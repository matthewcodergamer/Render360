...
function xex(base,encryptedBody){const h=Buffer.alloc(0x400),s=0x100,ffi=0x60,exec=0x90;h.write('XEX2',0,'ascii');p32be(h,4,1);p32be(h,8,0x400);p32be(h,0x10,s);p32be(h,0x14,4);let p=0x18;for(const[k,v]of[[0x3ff,ffi],[0x10100,base+0x1000],[0x10201,base],[0x40006,exec]]){p32be(h,p,k);p32be(h,p+4,v);p+=8}p32be(h,ffi,8);p16be(h,ffi+4,1);p16be(h,ffi+6,0);p32be(h,exec,0xAABBCCDD);p32be(h,exec+0x0c,0x584108CE);p32be(h,s,0x19c);p32be(h,s+4,0x2000);p32be(h,s+0x110,base);p32be(h,s+0x178,0xffffffff);p32be(h,s+0x17c,0x08000000);p32be(h,s+0x180,1);p32be(h,s+0x184,0x10|1);return Buffer.concat([h,encryptedBody]);}
async function run(seed,base){const plain=pe(base),session=Buffer.from(Array.from({length:16},(_,i)=>(seed+i*29)&255)),encryptedSecurityKey=aes(retail,session),cipher=aes(session,plain),defaultXex=xex(base,cipher);const r=await handoffDefaultXex({core,bootstrap,defaultXex,encryptedSecurityKey,entryBytes:8});const entry=(base+0x1000)>>>0;if(r.status!==1||r.entry!==entry||!r.hir||r.handoffBytes!==8||r.preparedBytes!==plain.length)throw new Error(`controller telemetry mismatch ${JSON.stringify(r)}`);if(r.executionStatus!==3||r.runtimeBoundary!=='guest-return'||r.executionInstructions===0||r.executionR3Hex!=='0x1')throw new Error(`entry execution telemetry mismatch ${JSON.stringify(r)}`);if(r.translatedFunctionCount===0||r.firstTranslatedFunction!==entry)throw new Error(`translated function telemetry mismatch ${JSON.stringify(r)}`);return r;}
...
console.log('DEFAULT_XEX_ENTRY_EXECUTION=PASS');
console.log('DEFAULT_XEX_RUNTIME_BOUNDARY_TELEMETRY=PASS');
