...
const req=['r360_kernel_runtime_reset','r360_guest_thread_create','r360_guest_thread_current','r360_guest_thread_set_current','r360_guest_thread_suspend','r360_guest_thread_resume','r360_guest_thread_terminate','r360_guest_thread_next_runnable','r360_guest_thread_state','r360_guest_thread_exit_code','r360_guest_thread_entry','r360_guest_thread_context','r360_guest_thread_flags','r360_guest_thread_stack_size','r360_guest_thread_stack_base','r360_guest_thread_stack_top','r360_guest_thread_stack_mapped','r360_guest_runtime_status','r360_guest_tls_alloc','r360_guest_tls_free','r360_guest_tls_set','r360_guest_tls_get','r360_sparse_guest_memory_read_u8','r360_sparse_guest_memory_write_u8','r360_sparse_guest_memory_last_fault_code'];for(const n of req)if(typeof pick(n)!=='function')throw new Error(`missing runtime critic export ${n}`);
...
if((pick('r360_guest_thread_entry')(a)>>>0)!==0x82001000||(pick('r360_guest_thread_context')(a)>>>0)!==0x1111||(pick('r360_guest_thread_flags')(a)>>>0)!==0xA5)throw new Error('thread execution metadata lost');
const stackBaseA=pick('r360_guest_thread_stack_base')(a)>>>0;const stackTopA=pick('r360_guest_thread_stack_top')(a)>>>0;const stackBaseB=pick('r360_guest_thread_stack_base')(b)>>>0;if(!stackBaseA||!stackTopA||!stackBaseB||stackBaseA===stackBaseB)throw new Error('thread stacks not independently mapped');if((stackTopA&15)!==0||stackTopA<=stackBaseA||stackTopA>=stackBaseA+0x4000)throw new Error('thread stack top is not a valid aligned in-stack PPC r1 seed');if((pick('r360_guest_thread_stack_mapped')(a)>>>0)!==1||(pick('r360_guest_thread_stack_mapped')(b)>>>0)!==1)throw new Error('thread stack mapping telemetry failed');
...
console.log('GUEST_RUNTIME_THREAD_CREATE=PASS');console.log('GUEST_RUNTIME_THREAD_STACK_MAPPING=PASS');console.log('GUEST_RUNTIME_DEFAULT_STACK_256K=PASS');console.log('GUEST_RUNTIME_THREAD_STACK_GUARDS=PASS');console.log('GUEST_RUNTIME_THREAD_METADATA=PASS');
...
if(!(pick('r360_guest_thread_terminate')(a,0xDEAD)>>>0))throw new Error('terminate failed');if((pick('r360_guest_thread_state')(a)>>>0)!==4||(pick('r360_guest_thread_exit_code')(a)>>>0)!==0xDEAD)throw new Error('termination telemetry wrong');if((pick('r360_guest_thread_stack_mapped')(a)>>>0)!==0)throw new Error('terminated thread retained mapped stack');if((pick('r360_sparse_guest_memory_write_u8')(stackBaseA,1)>>>0)!==0||(pick('r360_sparse_guest_memory_last_fault_code')()>>>0)!==1)throw new Error('terminated thread stack remained accessible');if((pick('r360_guest_tls_set')(a,slot,7)>>>0)!==0)throw new Error('terminated thread accepted TLS mutation');
console.log('GUEST_RUNTIME_TERMINATION_FAIL_CLOSED=PASS');console.log('GUEST_RUNTIME_THREAD_STACK_RELEASE=PASS');
...
const c=pick('r360_guest_thread_create')(0x82003000,0,0x4000,0)>>>0;if(!c||c===a)throw new Error('generation handle reuse failed');if((pick('r360_guest_thread_set_current')(a)>>>0)!==0)throw new Error('stale handle was accepted');if((pick('r360_guest_thread_set_current')(c)>>>0)!==1)throw new Error('replacement thread invalid');if((pick('r360_guest_thread_stack_base')(c)>>>0)!==stackBaseA||(pick('r360_guest_thread_stack_mapped')(c)>>>0)!==1)throw new Error('replacement thread did not safely reuse released stack slot');
