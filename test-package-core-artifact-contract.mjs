...
const required=[
  ...
  'r360_stfs_extract_begin','r360_stfs_extract_status','r360_stfs_extract_bytes_total','r360_stfs_extract_bytes_done',
  ...
  'r360_xex_pe_decode','r360_xex_pe_status','r360_xex_pe_entry_rva','r360_xex_pe_image_base',
];
const missing=required.filter(name=>!exports.has(name));
if(missing.length){console.error('PACKAGE_CORE_ARTIFACT FAIL');console.error(`missing exports (${missing.length}): ${missing.join(', ')}`);process.exit(1);}
const instance=new WebAssembly.Instance(module,{});
const build=instance.exports.r360_build_version()>>>0;
const abi=instance.exports.r360_abi_version()>>>0;
assert.ok(build>=32,`checked-in package core build ${build} is below synchronized V32 extractor contract`);
assert.ok(abi>=0x00030002,`checked-in package core ABI 0x${abi.toString(16)} is below 0x00030002`);
const features=instance.exports.r360_feature_bits()>>>0;
assert.ok(features&(1<<11),'package core feature bits do not advertise complete native STFS extraction');
assert.ok(features&(1<<12),'package core feature bits do not advertise strict STFS chain validation');
console.log(`PACKAGE_CORE_ARTIFACT PASS build=${build} abi=0x${abi.toString(16)} native_stfs=yes features=0x${features.toString(16)}`);
