...
function makeStfs(){
  ...
  e=0xD000;ascii(bytes,e,'default.xex');bytes[e+0x28]=11;put24le(bytes,e+0x29,2);put24le(bytes,e+0x2C,2);put24le(bytes,e+0x2F,3);put16be(bytes,e+0x32,0xFFFF);put32be(bytes,e+0x34,xex.length);bytes.set(xex.subarray(0,0x1000),0xE000);bytes.set(xex.subarray(0x1000),0xF000);return{bytes,xex};
}
...
const required=['memory','r360_build_version','r360_io_ptr','r360_io_capacity','r360_probe_container','r360_inspect_xex','r360_xex_status','r360_xex_entry_point','r360_xex_image_base','r360_xex_title_id','r360_xex_media_id','r360_stfs_mount_begin','r360_stfs_submit_read','r360_stfs_request_pending','r360_stfs_request_offset_lo','r360_stfs_request_offset_hi','r360_stfs_request_size','r360_stfs_mount_status','r360_stfs_entry_count','r360_stfs_default_xex_index','r360_stfs_default_xex_kind','r360_stfs_extract_begin','r360_stfs_extract_status','r360_stfs_extract_bytes_total','r360_stfs_extract_bytes_done','r360_stfs_extract_blocks_done'];
...
status=e.r360_stfs_extract_begin(xexIndex)>>>0;const captured=new Uint8Array(image.xex.length);let extractReads=0;
while((e.r360_stfs_request_pending()>>>0)!==0){if(++extractReads>64)throw new Error('STFS extraction request loop runaway');const off=u64(e.r360_stfs_request_offset_lo(),e.r360_stfs_request_offset_hi()),size=e.r360_stfs_request_size()>>>0,dest=e.r360_stfs_extract_bytes_done()>>>0,chunk=image.bytes.slice(off,off+size);if(chunk.length!==size)throw new Error(`STFS extraction short read @0x${off.toString(16)}`);if(dest<captured.length)captured.set(chunk.subarray(0,Math.min(chunk.length,captured.length-dest)),dest);stage(chunk);status=e.r360_stfs_submit_read(chunk.length)>>>0}
if((e.r360_stfs_extract_status()>>>0)!==2)throw new Error(`default.xex extraction failed: status=${e.r360_stfs_extract_status()>>>0}`);if((e.r360_stfs_extract_bytes_total()>>>0)!==image.xex.length||(e.r360_stfs_extract_bytes_done()>>>0)!==image.xex.length)throw new Error('default.xex extraction byte count mismatch');if((e.r360_stfs_extract_blocks_done()>>>0)!==2)throw new Error(`expected two XEX blocks, got ${e.r360_stfs_extract_blocks_done()>>>0}`);if(!Buffer.from(captured).equals(Buffer.from(image.xex)))throw new Error('complete default.xex capture differs from package payload');

stage(captured);if((e.r360_inspect_xex(captured.length)>>>0)!==1||(e.r360_xex_status()>>>0)!==1)throw new Error('captured XEX structural inspection failed');if((e.r360_xex_entry_point()>>>0)!==0x82001234||(e.r360_xex_image_base()>>>0)!==0x82000000)throw new Error('XEX execution metadata mismatch');if((e.r360_xex_title_id()>>>0)!==0x584108CE||(e.r360_xex_media_id()>>>0)!==0xAABBCCDD)throw new Error('XEX execution info mismatch');
...
console.log('PACKAGE_XEX_FOUNDATION=PASS');console.log(`core_version=${e.r360_build_version()>>>0}`);console.log(`mount_reads=${mountReads}`);console.log(`extract_reads=${extractReads}`);console.log(`default_xex_bytes=${image.xex.length}`);console.log(`default_xex_blocks=${e.r360_stfs_extract_blocks_done()>>>0}`);console.log('xex_entry=0x82001234');console.log('PASS: LIVE/PIRS/CON classification, STFS header/directory/hash-chain traversal, complete fragmented multi-block default.xex extraction, byte-for-byte reconstruction and XEX structural metadata inspection are runtime-gated.');
