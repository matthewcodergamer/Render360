...
assert.match(storage,/REQUIRED_PORTAL_STACK_REPAIR_VERSION=4/);
assert.match(storage,/assertFreshPortalRuntime/);
assert.match(storage,/runtimeStackRepairVersion/);
assert.match(storage,/runtimeProfile/);
assert.match(storage,/Relink using the newest Portal runtime ZIP/);
...
console.log('PC_OPFS_STALE_PORTAL_RUNTIME_REJECT=PASS');
