...
const pcStorage=fs.readFileSync('storage/pc-persistent-storage.js','utf8');
const serviceWorker=fs.readFileSync('render360-sw.js','utf8');
...
assert.match(build,/Render360 Source GOT sync/);
...
assert.match(adapter,/ignoredGameBinary/);
assert.match(adapter,/exe\|dll\|so\|dylib/);
...
assert.match(worker,/blockedGameBinary/);
assert.match(worker,/portal-native-module-filter/);
assert.match(worker,/\['-novid','-nojoy'\]/);
assert.match(worker,/blockedNativeFiles:blockedCount/);
assert.match(pcStorage,/exe\|dll\|so\|dylib/);
assert.match(pcStorage,/nativeGameBinariesFiltered:true/);
assert.match(pcStorage,/Old saved installs may still list native/);
assert.match(serviceWorker,/VERSION='75'/);
...
console.log('PORTAL_SOURCE_DLSYM_REQUIRED_GOT_CATCHUP=PASS');
...
console.log('PORTAL_SOURCE_NATIVE_GAME_MODULE_FILTER=PASS');
console.log('PORTAL_SOURCE_NOVID_NOJOY=PASS');
console.log('PORTAL_SOURCE_SAFARI_CACHE_REVISION=PASS');
