...
const req=['memory','r360_xex_guest_mapper_input_buffer','r360_xex_guest_mapper_input_capacity','r360_pe_guest_load','r360_pe_guest_status','r360_pe_guest_entry_address','r360_title_handoff_reset','r360_title_handoff_translate_entry','r360_title_handoff_translate_scanned_entry','r360_title_handoff_status','r360_title_handoff_entry_address','r360_title_handoff_bytes','r360_title_handoff_hir_instructions','r360_ppc_probe_status','r360_ppc_probe_guest_base','r360_ppc_probe_loaded_size','r360_ppc_probe_correctness_status','r360_ppc_probe_correctness_r3'];
...
function makeProgramPE(base,scanned=false){
  ...
  if(scanned){
    // The meaningful third instruction starts after byte 8. The scanned path
    // must discover the later BLR and execute r3=3 rather than stopping at the
    // historical two-instruction probe boundary.
    be32(a,0x200,0x38600001);be32(a,0x204,0x38600002);be32(a,0x208,0x38600003);be32(a,0x20c,0x4E800020);
  }else{
...
const loadPe=(base,scanned=false)=>{const pe=makeProgramPE(base,scanned);if(pe.length>cap)throw new Error('PE staging overflow');new Uint8Array(instance.exports.memory.buffer,input,pe.length).set(pe);if((pick('r360_pe_guest_load')(input,pe.length)>>>0)!==1)throw new Error(`PE guest load failed 0x${(pick('r360_pe_guest_status')()>>>0).toString(16)}`);const entry=(base+0x1000)>>>0;eq(pick('r360_pe_guest_entry_address')(),entry,'PE entry');return entry;};
const run=base=>{
  const entry=loadPe(base,false);
...
// The compatibility handoff remains deliberately bounded/aligned.
...
// Browser-grade handoff pages the mapped executable PE section and lets
// upstream Xenia's PPCScanner discover the function boundary.
const scannedEntry=loadPe(0x86000000,true);
pick('r360_title_handoff_reset')();const scannedHir=pick('r360_title_handoff_translate_scanned_entry')()>>>0;if(!scannedHir)throw new Error(`scanned entry handoff failed 0x${(pick('r360_title_handoff_status')()>>>0).toString(16)}`);
eq(pick('r360_title_handoff_status')(),1,'scanned handoff status');eq(pick('r360_title_handoff_entry_address')(),scannedEntry,'scanned handoff entry');
const scannedBytes=pick('r360_title_handoff_bytes')()>>>0;if(scannedBytes<=8)throw new Error(`scanned handoff remained probe-sized ${scannedBytes}`);
eq(pick('r360_ppc_probe_correctness_status')(),3,'scanned HIR execution status');
if(BigInt.asUintN(64,pick('r360_ppc_probe_correctness_r3')())!==3n)throw new Error('scanned entry did not execute instruction beyond byte 8');
console.log('PREPARED_PE_SCANNED_ENTRY_RX_PAGING=PASS');
console.log('PREPARED_PE_SCANNED_ENTRY_XENIA_SCAN=PASS');
console.log('PREPARED_PE_SCANNED_ENTRY_BEYOND_8_BYTES=PASS');
