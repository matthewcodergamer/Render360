...
# Opcode truth for the pinned Xenia revision:
# CALL_INDIRECT is HIR opcode 9. HIR opcode 37 is LOAD_OFFSET. The native HIR
# executor already implements CALL_INDIRECT, so the Braid failure at opcode 37
# must be treated as a guest-memory dependency / generated-WASM coverage gap,
# not patched with a duplicate CALL_INDIRECT switch case.
replace_once(
...
# Export a narrow fail-closed scalar guest-memory load bridge for generated
# child Wasm modules. Child modules share the bootstrap memory, but the Xbox
# 32-bit virtual address space is represented by SparseGuestMemory rather than
# a fake 4 GiB linear-memory mapping, so generated code must ask the parent for
# scalar loads instead of indexing WebAssembly.Memory with a guest VA.
replace_once(
...
replace_once(
    ...
    '''  EmitU32Leb(types, 1);\n  types.push_back(0x7E);\n  // type 2: guest_load(address, size, flags) -> i64\n  types.push_back(0x60);\n  EmitU32Leb(types, 3);\n  types.push_back(0x7F);\n  types.push_back(0x7F);\n  types.push_back(0x7F);\n  EmitU32Leb(types, 1);\n  types.push_back(0x7E);\n  EmitSection(module, 1, types);''')
...
# Wire the generated guest_load import to the authoritative sparse-memory bridge
# in the persistent browser execution session.
replace_once(
    'render360-browser-ppc-session.mjs',
...
# Strengthen the generated-call regression so every child module must expose the
# new strict import, and so the exact Braid shape (LOAD_OFFSET -> mtctr -> bctrl
# -> xam.xex 0x28B) is executed through generated Wasm and the live kernel bridge.
replace_once(
