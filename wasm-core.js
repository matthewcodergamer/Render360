...
export class Render360Core {
  ...
  async init() {
    ...
    try{
      const response=await fetchWithTimeout(this.url,{cache:'no-store',timeoutMs:2500});if(!response.ok)throw new Error(`HTTP ${response.status}`);
  ...
  stageBytes(bytes){const src=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes),cap=this.ioCapacity();if(src.byteLength>cap)throw new Error(`WASM staging overflow: ${src.byteLength} > ${cap}`);new Uint8Array(this.exports.memory.buffer,this.ioPtr(),src.byteLength).set(src);return {ptr:this.ioPtr(),cap,bytesRead:src.byteLength};}
  async stageSlice(file,offset=0,length=this.ioCapacity()){const safeOffset=Math.max(0,Number(offset));if(!Number.isSafeInteger(safeOffset))throw new Error('File offset exceeds JavaScript safe integer range');const available=Math.max(0,file.size-safeOffset),take=Math.min(this.ioCapacity(),Math.max(0,Number(length)),available),bytes=new Uint8Array(await file.slice(safeOffset,safeOffset+take).arrayBuffer());this.stageBytes(bytes);return {ptr:this.ioPtr(),cap:this.ioCapacity(),bytesRead:bytes.length,offset:safeOffset,bytes};}
  async stageFilePrefix(file){return this.stageSlice(file,0,this.ioCapacity())}
  readAscii(ptr,length){if(!ptr||!length)return '';const bytes=new Uint8Array(this.exports.memory.buffer,ptr>>>0,length>>>0);let out='';for(const b of bytes)out+=String.fromCharCode(b);return out;}
  combineU64(lo,hi){const value=(hi>>>0)*U32+(lo>>>0);if(!Number.isSafeInteger(value))throw new Error('64-bit file value exceeds JavaScript safe integer range');return value;}
  splitU64(value){if(!Number.isSafeInteger(value)||value<0)throw new Error('Invalid file size');return {lo:(value%U32)>>>0,hi:Math.floor(value/U32)>>>0};}

  readXexInspection(inspectStatus=this.exports.r360_xex_status()>>>0){
    return {inspectStatus,moduleFlags:this.exports.r360_xex_module_flags()>>>0,headerSize:this.exports.r360_xex_header_size()>>>0,securityOffset:this.exports.r360_xex_security_offset()>>>0,headerCount:this.exports.r360_xex_header_count()>>>0,entryPoint:this.exports.r360_xex_entry_point()>>>0,imageBase:this.exports.r360_xex_image_base()>>>0,systemFlags:this.exports.r360_xex_system_flags()>>>0,titleId:this.exports.r360_xex_title_id()>>>0,mediaId:this.exports.r360_xex_media_id()>>>0,imageSize:this.exports.r360_xex_image_size()>>>0,loadAddress:this.exports.r360_xex_load_address()>>>0,region:this.exports.r360_xex_region()>>>0,allowedMediaTypes:this.exports.r360_xex_allowed_media_types()>>>0,pageDescriptorCount:this.exports.r360_xex_page_descriptor_count()>>>0,encryptionType:this.exports.r360_xex_encryption_type()>>>0,compressionType:this.exports.r360_xex_compression_type()>>>0,importsOffset:this.exports.r360_xex_import_libraries_offset()>>>0,executionInfoOffset:this.exports.r360_xex_execution_info_offset()>>>0,fileFormatInfoOffset:this.exports.r360_xex_file_format_info_offset()>>>0};
  }

  async probeFile(file){const magic=await this.stageSlice(file,0,Math.min(file.size,64)),kind=this.exports.r360_probe_container(magic.bytesRead)>>>0,result={kind,...magic,xex:null,stfs:null};if(kind===1||kind===2){const staged=await this.stageFilePrefix(file);result.bytesRead=staged.bytesRead;const inspectStatus=this.exports.r360_inspect_xex(staged.bytesRead)>>>0;result.xex=this.readXexInspection(inspectStatus);}return result;}
  inspectXexBytes(bytes){const src=bytes instanceof Uint8Array?bytes:new Uint8Array(bytes),take=Math.min(src.byteLength,this.ioCapacity());this.stageBytes(src.subarray(0,take));const inspectStatus=this.exports.r360_inspect_xex(take)>>>0;return this.readXexInspection(inspectStatus);}

  readStfsSnapshot(){
    const e=this.exports,status=e.r360_stfs_mount_status()>>>0,namePtr=e.r360_stfs_display_name_ptr()>>>0,nameLen=e.r360_stfs_display_name_length()>>>0;
    return {status,statusName:STFS_STATUS[status]||`Status ${status}`,packageKind:e.r360_stfs_package_kind()>>>0,headerSize:e.r360_stfs_header_size()>>>0,contentType:e.r360_stfs_content_type()>>>0,metadataVersion:e.r360_stfs_metadata_version()>>>0,contentSize:this.combineU64(e.r360_stfs_content_size_lo(),e.r360_stfs_content_size_hi()),titleId:e.r360_stfs_title_id()>>>0,mediaId:e.r360_stfs_media_id()>>>0,volumeType:e.r360_stfs_volume_type()>>>0,descriptorLength:e.r360_stfs_descriptor_length()>>>0,descriptorVersion:e.r360_stfs_descriptor_version()>>>0,descriptorFlags:e.r360_stfs_descriptor_flags()>>>0,readOnly:!!((e.r360_stfs_descriptor_flags()>>>0)&1),dataFileCount:e.r360_stfs_data_file_count()>>>0,fileTableBlockCount:e.r360_stfs_file_table_block_count()>>>0,fileTableBlockNumber:e.r360_stfs_file_table_block_number()>>>0,totalBlockCount:e.r360_stfs_total_block_count()>>>0,freeBlockCount:e.r360_stfs_free_block_count()>>>0,directoryBlocksRead:e.r360_stfs_directory_blocks_read()>>>0,entryCount:e.r360_stfs_entry_count()>>>0,defaultXexIndex:e.r360_stfs_default_xex_index()>>>0,defaultXexKind:e.r360_stfs_default_xex_kind()>>>0,warnings:e.r360_stfs_warnings()>>>0,displayName:this.readAscii(namePtr,nameLen)};
  }

  readStfsEntries(){const e=this.exports,count=e.r360_stfs_entry_count()>>>0,entries=[];for(let i=0;i<count;i++){const ptr=e.r360_stfs_entry_name_ptr(i)>>>0,len=e.r360_stfs_entry_name_length(i)>>>0,flags=e.r360_stfs_entry_flags(i)>>>0;entries.push({index:i,name:this.readAscii(ptr,len),flags,contiguous:!!(e.r360_stfs_entry_is_contiguous(i)>>>0),directory:!!(e.r360_stfs_entry_is_directory(i)>>>0),validBlocks:e.r360_stfs_entry_valid_blocks(i)>>>0,allocatedBlocks:e.r360_stfs_entry_allocated_blocks(i)>>>0,startBlock:e.r360_stfs_entry_start_block(i)>>>0,parentIndex:e.r360_stfs_entry_parent_index(i)>>>0,length:e.r360_stfs_entry_length(i)>>>0});}return entries;}
  readStfsExtractSnapshot(){const e=this.exports;if(typeof e.r360_stfs_extract_status!=='function')return null;const status=e.r360_stfs_extract_status()>>>0;return {status,statusName:STFS_EXTRACT_STATUS[status]||`Status ${status}`,entryIndex:e.r360_stfs_extract_entry_index()>>>0,currentBlock:e.r360_stfs_extract_current_block()>>>0,logicalOffset:e.r360_stfs_extract_logical_offset()>>>0,bytesTotal:e.r360_stfs_extract_bytes_total()>>>0,bytesDone:e.r360_stfs_extract_bytes_done()>>>0,blocksDone:e.r360_stfs_extract_blocks_done()>>>0,contiguous:!!(e.r360_stfs_extract_is_contiguous()>>>0)};}
