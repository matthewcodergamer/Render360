...
if missing:
    raise SystemExit('Run ./fetch-xenia.sh first; missing: ' + ', '.join(str(p) for p in missing))
...
for name, value in required_xex.items():
    if not re.search(rf'\b{name}\s*=\s*{re.escape(value)}', texts[info]):
        failed.append(f'{name} != {value}')
for token in ['magic', 'module_flags', 'header_size', 'security_offset', 'header_count']:
    if not re.search(rf'\b{token}\b', texts[info]):
        failed.append(f'xex2_info.h missing {token}')
for token in ['kXEX1Signature', 'kXEX2Signature', 'GetOptHeader']:
    if token not in texts[module]:
        failed.append(f'xex_module.h missing {token}')
for sig, value in [('kCon', '0x434F4E20'), ('kPirs', '0x50495253'), ('kLive', '0x4C495645')]:
    if not re.search(rf'\b{sig}\s*=\s*{value}', texts[stfs]):
        failed.append(f'STFS package magic {sig} drifted')
for token in [
    ...
    if token not in texts[stfs]:
        failed.append(f'stfs_xbox.h missing {token}')
for token in [
    ...
    if token not in texts[device]:
        failed.append(f'stfs_container_device.cc missing {token}')
if failed:
    print('Xenia contract drift detected:')
    for item in failed:
        print(' -', item)
    sys.exit(1)
print('PASS: XEX + native STFS portability contract still matches upstream Xenia')
