...
if not XENIA.exists(): fail('upstream Xenia is missing; run fetch-xenia.sh first')
for path in EXPECTED_UPSTREAM:
    if not (XENIA / path).is_file(): fail(f'missing required upstream translation source: {path}')
for path in EXPECTED_RENDER360:
    if not (ROOT / path).is_file(): fail(f'missing required browser translation source: {path}')

build_text = BUILD.read_text(encoding='utf-8')
for path in EXPECTED_UPSTREAM:
    if f'"{path}"' not in build_text: fail(f'build matrix no longer includes: {path}')
for path in EXPECTED_RENDER360:
    label = path.replace('src/xenia_web_bootstrap/', 'render360/')
    if label not in build_text: fail(f'build matrix no longer includes browser unit: {label}')

link_text = LINK.read_text(encoding='utf-8')
for marker in ['ERROR_ON_UNDEFINED_SYMBOLS=1', 'r360_ppc_probe_translate', 'r360_ppc_probe_correctness_status',
               'src_xenia_cpu_compiler_passes_data_flow_analysis_pass.cc.o',
               'src_xenia_cpu_compiler_passes_value_reduction_pass.cc.o']:
    if marker not in link_text: fail(f'strict link contract marker missing: {marker}')

test_text = TEST.read_text(encoding='utf-8')
for marker in REQUIRED_TEST_MARKERS:
    if marker not in test_text: fail(f'required end-to-end PPC runtime category disappeared: {marker}')
...
if missing_passes: fail('new/untracked upstream compiler pass(es): ' + ', '.join(missing_passes))
if extra_passes: fail('foundation manifest references missing compiler pass(es): ' + ', '.join(extra_passes))

emitters = sorted(p.name for p in (XENIA / 'src/xenia/cpu/ppc').glob('ppc_emit_*.cc'))
required_emitters = sorted(['ppc_emit_alu.cc','ppc_emit_control.cc','ppc_emit_memory.cc','ppc_emit_fpu.cc','ppc_emit_altivec.cc'])
if any(name not in emitters for name in required_emitters): fail('one or more required PPC emitter categories disappeared from upstream Xenia')
...
print('PASS: frontend, translator, scanner, all five PPC emit categories, HIR, every current upstream compiler-pass implementation, strict link contract, and representative real-PPC runtime categories are locked by drift detection.')
