...
REQUIRED_TEST_MARKERS = [
    'runtime-addi-r4-plus-5',
    'branch-equal-taken',
    'stw-lwz-xenia-memory-roundtrip',
    'ctr-bdnz-loop-three-iterations',
    'direct-bl-callee-blr-caller',
    'nested-bl-two-level-call-return',
    'fpu-lfd-fadd-stfd-three',
    'fpu-lfd-fsub-stfd-three',
    'fpu-lfd-fmul-stfd-three',
    'vmx-lvx-vaddubm-stvx-bytes',
]
...
# Keep the closure strict. If these disappear, a green compile can no longer be
# interpreted as the translation foundation being complete.
...
for marker in ['ERROR_ON_UNDEFINED_SYMBOLS=1', 'r360_ppc_probe_translate', 'r360_ppc_probe_correctness_status']:
    if marker not in link_text:
        fail(f'strict link contract marker missing: {marker}')
...
# Verify all upstream compiler pass implementation files in the current Xenia
# tree are intentionally represented. This catches drift when Xenia adds a new
# portable pass that the browser foundation would otherwise silently omit.
...
# Verify the five semantic PPC emitter categories remain present and compiled.
...
print('PASS: frontend, translator, scanner, all five PPC emit categories, HIR, current compiler pass chain, strict link contract, and representative real-PPC runtime categories are locked by drift detection.')
